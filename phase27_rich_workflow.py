...
def _cf_html():
    ...
    k.GlobalLock.restype=ctypes.c_void_p;k.GlobalSize.restype=ctypes.c_size_t
    ...
    try:
        ...
        m=re.search(r'<!--StartFragment-->(.*?)<!--EndFragment-->',s,re.I|re.S);return m.group(1) if m else s
...
def _apply_runs(app,ranges):
    try:
        from sitecustomize import _widget_order;widgets=_widget_order(app)
...
def _paste_image_near_cursor(app,w):
    try:
        ...
        bb=w.bbox('insert');x=w.winfo_x()+(bb[0] if bb else 0);y=w.winfo_y()+(bb[1] if bb else 0);z=max(.25,float(getattr(app,'_zoom',1.0)))
        ...
        ph=ImageTk.PhotoImage(im);label=tk.Label(page,image=ph,bd=1,relief='solid');label.place(x=x,y=y,width=im.width,height=im.height)
        app._pm_image_refs=getattr(app,'_pm_image_refs',[])+[ph]
        obj=app.document.add_image(app.document.pages[pi],Rect(x/z,y/z,im.width/z,im.height/z),path);app._pm_image_widgets=getattr(app,'_pm_image_widgets',{});app._pm_image_widgets[obj.id]=label
        try:import phase26_image_resize as r;r.bind_image(app,obj.id,label)
        except Exception:pass
        app._pm_selected_image=obj.id;app.dirty=True;app.status.config(text='Image pasted at insertion point — drag/resize handles');return True
...
def paste_rich(app,w=None):
    ...
    if source:
        try:
            ...
            if images or _paste_image_near_cursor(app,w):pass
            app.status.config(text='Rich paste: bulk text + Unicode math + sup/sub + basic formatting preserved');return 'break'
        except Exception:pass
    try:
...
def install(app):
    try:
        import phase25_editor_fixes as p25;p25.paste=lambda a,w=None:paste_rich(a,w);app.paste=lambda w=None:paste_rich(app,w);app._pm27_rich_paste=True
    except Exception:pass
